...
export const haveGit = () => new Promise((resolve, reject) => {
    let timeout = setTimeout(_ => reject(new Error('Time out')), 10 * 1000);
    let nowVersion = spawn.sync('git', ['--version']).stdout.toString();
    if (nowVersion.includes('git version')) {
        clearTimeout(timeout);
    }
    resolve(nowVersion.includes('git version'));
});